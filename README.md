# RedMonk – App Network Activity Report Generator

## About

RedMonk is an Android app that shows network usage of apps and generates network-app activity report as PDF, and helps users understand which apps send & receive data.

## Features

* app network monitoring
* Upload / Download data per app
* Protocol display (TCP / UDP)
* Time-based activity logging
* generate network activity report as PDF
* it checks for anomoly patterns - basically a very simple model

## PDF Generator

* it generates pdf with a network activity
* it has a report like strcture with day/date/year
* simple user-friendly pdf to understand apps activity
